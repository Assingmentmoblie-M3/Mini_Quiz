...
class UserScreen extends StatefulWidget {
  ...
  const UserScreen({this.id,this.isEdit, this.email, this.roleId, this.status, super.key});
  ...
  Future<void> _saveUser() async {
    ...
    if (isEdit) {
      ...
      success = await provider.updateUser(
        ...
        roleId: widget.roleId ?? 1,
        status: widget.status ?? 1,
      );
    } else {
      // 🔥 add mode
      success = await provider.addUser(email: _emailController.text.trim());
    }
    ...
    if (success && isEdit) {
      Navigator.pop(context); // បង្ហាញត្រឡប់ទៅ ViewUserScreen
    } else if (success) {
      _emailController.clear(); // Add mode → reset field
    }
  }
  /*Future<void> _saveUser() async {
...
class _UserScreenState extends State<UserScreen> {
  ...
  Future<void> _saveUser() async {
    ...
    if (success) {
      ...
      Navigator.pop(context); // 🔥 ត្រឡប់ទៅ ViewUserScreen
    } else {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
